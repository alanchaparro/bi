...
MYSQL_INCREMENTAL_HINTS = {
    ...
    "cobranzas": {
        "updated_col": "Actualizado_al",
        "id_col": "id",
        "format": "%Y-%m-%d %H:%i:%s",
    },
    "cartera": {
        "updated_col": "fecha_cierre",
        "id_col": "id_contrato",
        "format": "%Y/%m/%d",
    },
    "contratos": {
...
def _query_variant_for_domain(domain: str) -> str:
    ...
    if raw not in {"v1", "v2"}:
        logger.warning(
            "[sync:%s] invalid query variant=%s, fallback=v1", domain_key, raw
        )
        return "v1"
...
RUNNING_JOB_STALE_GRACE_SECONDS = max(
    60, int(getattr(settings, "sync_running_stale_grace_seconds", 600) or 600)
)
...
def _load_sql_with_includes(
    ...
    *,
    ...
    return load_sql_with_includes(
        query_path, depth=depth, max_depth=max_depth, stack=stack
    )
...
def _semantic_refresh_month_batches(affected_months: set[str]) -> list[list[str]]:
    months = sorted(
        {
            str(m).strip()
            for m in (affected_months or set())
            if _month_serial(str(m).strip()) > 0
        },
        key=_month_serial,
    )
    if not months:
...
def _invalidate_when_months_overlap(
    filters: dict | None, target_months: set[str], *keys: str
) -> bool:
...
def _invalidate_cartera_cache(
    filters: dict | None, target_gestion_months: set[str]
) -> bool:
...
def _parse_payment_date(row: dict) -> date | None:
    ...
    return _parse_iso_date(
        _normalize_key(row, "date", "payment_date", "Actualizado_al")
    )
...
def _source_dedupe_key(normalized: dict) -> tuple:
    ...
    if domain == "cartera":
        close_date = (
            normalized.get("close_date") or normalized.get("gestion_month") or ""
        )
        return (
...
def _set_state(domain: str, updates: dict) -> None:
    ...
    with _state_lock:
        ...
        if started_at_dt is not None:
            ...
            rows_processed = float(current.get("rows_upserted") or 0.0) + float(
                current.get("rows_unchanged") or 0.0
            )
            throughput = (
                rows_read / elapsed if rows_read > 0 else rows_processed / elapsed
            )
            current["throughput_rows_per_sec"] = (
                round(throughput, 2) if throughput > 0 else 0.0
            )
        ...
        _persist_sync_run(
            db,
            {
                ...
                "throughput_rows_per_sec": float(
                    snapshot.get("throughput_rows_per_sec") or 0.0
                ),
                "eta_seconds": int(snapshot.get("eta_seconds") or 0),
                "current_query_file": snapshot.get("current_query_file")
                or _query_file_for(domain),
                ...
                "skipped_unchanged_chunks": int(
                    snapshot.get("skipped_unchanged_chunks") or 0
                ),
                "duplicates_detected": int(snapshot.get("duplicates_detected") or 0),
                "error": snapshot.get("error"),
                "started_at": (
                    datetime.fromisoformat(
                        str(snapshot.get("started_at")).replace("Z", "+00:00")
                    ).replace(tzinfo=None)
                ...
                "finished_at": (
                    datetime.fromisoformat(
                        str(snapshot.get("finished_at")).replace("Z", "+00:00")
                    ).replace(tzinfo=None)
...
def _resolve_mysql_connection_config(db: Session | None = None) -> dict[str, Any]:
    cfg = {
        ...
        "connection_timeout": max(
            5, int(getattr(settings, "mysql_connection_timeout_seconds", 20) or 20)
        ),
        "read_timeout": max(
            30, int(getattr(settings, "mysql_read_timeout_seconds", 600) or 600)
        ),
        "write_timeout": max(
            30, int(getattr(settings, "mysql_write_timeout_seconds", 120) or 120)
        ),
    ...
    try:
        ...
        try:
            if getattr(conn, "_socket", None) is not None:
                conn._socket.settimeout(
                    float(max(30, int(cfg.get("read_timeout") or 600)))
                )
        except Exception:
            pass
        cursor = conn.cursor(dictionary=True)
        try:
            ...
            domain_key = str(domain or "").strip().lower()
            query_variant = (
                _query_variant_for_domain(domain_key) if domain_key else "v1"
            )
            logger.info(
            ...
            if bool(settings.sync_mysql_incremental_pushdown) and domain:
                try:
                    effective_sql, effective_params, used_pushdown = (
                        _build_mysql_incremental_query(
                            domain=domain,
                            base_sql=query_text,
            ...
            try:
                ...
                if used_pushdown:
                    ...
                    raise
            batch_size = int(batch_size_override or 0) or _fetch_batch_size_for_domain(
                domain
            )
...
def _delete_target_window(
    db: Session, domain: str, mode: str, year_from: int | None, target_months: set[str]
) -> None:
...
def _fact_business_key_tuple(record: dict, domain: str) -> tuple:
    ...
    if domain == "cartera":
        return (
            ...
            str(record.get("close_date") or "")[:10]
    ...
    if domain == "cobranzas":
        ...
        return (
            str(record.get("contract_id") or ""),
            str(record.get("payment_date") or "")[:10]
...
def _filter_rows_changed_vs_postgres(
    db: Session, domain: str, rows: list[dict]
) -> list[dict]:
...
def _upsert_fact_rows(
    db: Session, domain: str, rows: list[dict], *, commit: bool = True
) -> tuple[int, int]:
    if not rows:
        return 0, 0
    model = FACT_TABLE_BY_DOMAIN[domain]
    ...
    if not use_on_conflict:
        ...
        if domain == "cartera":
            ...
            keys = [
                str(v.get("source_row_id") or "").strip()
                for v in values
                if str(v.get("source_row_id") or "").strip()
            ]
            if keys:
                for chunk in _iter_chunks(keys, size=3000):
                    db.query(CobranzasFact).filter(
                        CobranzasFact.source_row_id.in_(chunk)
                    ).delete(synchronize_session=False)
        elif domain == "eerr":
            keys = [
                (
...
def _effective_cartera_month_by_cutoff(
    db: Session, cutoff_months: list[str]
) -> dict[str, str]:
    """Resolve, for each cutoff month, the latest cartera month <= cutoff."""
    valid_cutoffs = sorted(
        {
            str(m).strip()
            for m in (cutoff_months or [])
            if _month_serial(str(m).strip()) > 0
        },
        key=_month_serial,
    )
    ...
    for cutoff in valid_cutoffs:
        ...
        while (
            idx < len(cartera_with_serial)
            and cartera_with_serial[idx][1] <= cutoff_serial
        ):
...
def _build_cartera_contract_year_expr():
    if engine.dialect.name == "postgresql":
        ...
        return case(
            (
                contract_month.op("~")(r"^\d{2}/\d{4}$"),
                cast(func.substring(contract_month, 4, 4), Integer),
            ),
...
def _refresh_cartera_corte_agg(
    db: Session, affected_months: set[str]
) -> tuple[int, int]:
...
def _refresh_cobranzas_cohorte_agg(
    db: Session, affected_months: set[str]
) -> tuple[int, int]:
    months = sorted(
        {
...
def _refresh_dim_negocio_contrato(
    db: Session, affected_months: set[str]
) -> tuple[int, int]:
...
def _refresh_analytics_rendimiento_agg(
    db: Session, affected_months: set[str]
) -> tuple[int, int]:
    return refresh_analytics_rendimiento_agg(
        db, affected_months, _month_serial, _canonical_via
    )


def _refresh_analytics_anuales_agg(
    db: Session, affected_months: set[str]
) -> tuple[int, int]:
...
def _refresh_source_freshness_snapshots(
    db: Session, last_job_id: str | None = None
) -> None:
    ...
    for attempt in range(2):
        ...
        try:
            ...
            if bool(payload.get("running")):
                ...
                if (
                    qrow is not None
                    and str(qrow.status or "").strip().lower() == "running"
                ):
...
def _persist_job_step(
    ...
    try:
        row = (
            ...
            .filter(
                SyncJobStep.job_id == job_id,
                SyncJobStep.domain == domain,
                SyncJobStep.step_name == step_name,
            )
        ...
        try:
            ...
            db.rollback()
            logger.exception(
                "[sync:%s:%s] failed to persist job step=%s status=%s",
                domain,
                job_id,
                step_name,
                status,
...
def _get_watermark(
    db: Session, domain: str, query_file: str, partition_key: str = "*"
) -> SyncWatermark | None:
    return (
...
def _upsert_watermark(
    ...
    *,
    ...
    row = _get_watermark(
        db, domain=domain, query_file=query_file, partition_key=partition_key
    )
    if row is None:
        row = SyncWatermark(
            domain=domain,
    ...
    *,
    ...
    for chunk_key, signal in chunk_signals.items():
        ...
        if row is None:
            ...
            continue
        if (
            str(row.chunk_hash or "") == chunk_hash
            and int(row.row_count or 0) == row_count
        ):
    ...
    try:
        ...
        for row in rows:
            ...
            if queue_row is not None:
                ...
                heartbeat_at = (
                    queue_row[1] or queue_row[2] or queue_row[3] or row.started_at
                )
                if (
                    queue_status == "running"
                    and heartbeat_at is not None
                    and (now - heartbeat_at).total_seconds()
            ...
            try:
                ...
                existing_log = []
            existing_log.append(
                f"Error: job_interrupted_on_restart ({now.isoformat()})"
            )
...
def _queue_has_running_or_pending(db: Session) -> bool:
    row = (
        db.query(SyncJob.id).filter(SyncJob.status.in_(["pending", "running"])).first()
    )
...
def _update_schedule_after_run_if_done(
    ...
    runs = (
        (db.query(SyncRun).filter(SyncRun.job_id.in_(job_ids)).all()) if job_ids else []
    )
    ...
    for j in jobs:
        ...
        if r and (
            ...
            or (str(getattr(r, "stage", "") or "").lower() == "failed")
        ):
            status = "failed"
            if r.error:
...
def _execute_job(
    ...
    try:
        ...
        range_months = (
            _month_range(close_month_from or "", close_month_to or "")
            if mode == "range_months"
            else []
        )
        ...
        if not query_path.exists():
            raise RuntimeError(
                f"No existe query para dominio {domain}: {query_path.name}"
            )
        ...
        _set_state(
            domain,
            {
                "stage": "connecting_mysql",
                "progress_pct": 8,
                "status_message": "Conectando a MySQL",
            },
        )
        _append_log(domain, "Conectando a MySQL...")
        partition_key = _watermark_partition_key(
            mode, year_from, close_month, close_month_from, close_month_to
        )
        ...
        wm_filter_updated_at = (
            watermark_row.last_updated_at if watermark_row is not None else None
        )
        wm_filter_source_id = (
            str(watermark_row.last_source_id or "").strip()
        ...
        if getattr(settings, "sync_precheck_enabled", True):
            if not _mysql_has_new_data(
                ...
                _append_log(
                    domain,
                    "Pre-check: sin datos nuevos, omitiendo sync (skipped_no_changes).",
                )
                ...
                _persist_job_step(
                    db, job_id, domain, "extract", "completed", {"rows_read": 0}
                )
                _persist_job_step(
                    db,
                    job_id,
                    ...
                    "normalize",
                    "completed",
                    {"rows_read": 0, "normalized": 0},
                )
                _persist_job_step(
                    db, job_id, domain, "replace_window", "completed", {"months": []}
                )
                _persist_job_step(
                    db,
                    job_id,
                    ...
                    "upsert",
                    "completed",
                    {"rows_upserted": 0, "rows_unchanged": 0},
                )
                _persist_job_step(
                    db,
                    job_id,
                ...
                logger.info(
                    "[sync:%s:%s] skipped_no_changes (pre-check)", domain, job_id
                )
                return
        ...
        effective_fetch_batch = (
            _low_impact_fetch_batch_size(default_fetch_batch)
            if low_impact_mode
            else default_fetch_batch
        )
        if low_impact_mode:
            _append_log(
                domain,
        ...
        _set_state(
            domain,
            {
                "stage": "normalizing",
                "progress_pct": 35,
                "status_message": "Normalizando filas",
            },
        )
        ...
        jsonl_flush_every = max(
            100, int(getattr(settings, "sync_jsonl_flush_every_rows", 1000) or 1000)
        )

        def _jsonl_line(record: dict) -> str:
            json_row = (
                record
                if "_sync_payload_parsed" not in record
                else {k: v for k, v in record.items() if k != "_sync_payload_parsed"}
            )
        ...
        def _flush_temp_rows(force: bool = False) -> None:
            if temp_rows_file is None:
                return
            if temp_rows_buffer and (
                force or len(temp_rows_buffer) >= jsonl_flush_every
            ):
        ...
        if domain in stream_to_disk_domains:
            if domain == "cartera":
                ...
                fd, temp_rows_path = tempfile.mkstemp(
                    prefix=f"sync_{domain}_", suffix=".jsonl"
                )
                os.close(fd)
                ...
                for row in batch:
                    ...
                    if wm_filter_updated_at is not None and raw_updated_at is not None:
                        ...
                        if (
                            ...
                            try:
                                should_skip_by_watermark = int(raw_source_id) <= int(
                                    wm_filter_source_id
                                )
                            except Exception:
                                should_skip_by_watermark = str(raw_source_id) <= str(
                                    wm_filter_source_id
                                )
                        if should_skip_by_watermark:
                            ...
                            continue
                    mode_month = str(
                        n.get("close_month") or n.get("gestion_month") or ""
                    )
                    if domain != "cartera":
                        mode_month = str(n.get("gestion_month") or "")
                    if not _matches_mode(
                        mode_month, mode, year_from, close_month, range_months_set
                    ):
                        continue
                    source_months.add(n["gestion_month"])
                    if raw_updated_at is not None and (
                        wm_last_updated_at is None
                        or raw_updated_at > wm_last_updated_at
                    ):
                        ...
                        if wm_last_source_id is None:
                            ...
                            try:
                                ...
                                if raw_source_id > wm_last_source_id:
                                    wm_last_source_id = raw_source_id
                    chunk_signals[n["gestion_month"]] = _chunk_signal_update(
                        chunk_signals.get(n["gestion_month"]), n["source_hash"]
                    )
                    normalized_count += 1
                        ...
                        if month_key:
                            if month_key not in temp_file_by_month:
                                fd, path = tempfile.mkstemp(
                                    prefix=f"sync_{domain}_{month_key.replace('/', '_')}_",
                                    suffix=".jsonl",
                                )
                                os.close(fd)
                                temp_rows_by_month[month_key] = path
                                temp_file_by_month[month_key] = open(
        ...
        _persist_job_step(
            ...
            "normalize",
            "completed",
            {"rows_read": source_rows, "normalized": normalized_count},
        )
        ...
        _set_state(
            domain,
            {
                ...
                "chunk_key": ",".join(sorted(target_months, key=_month_serial))
                if target_months
                else "*",
            },
        )
        if skipped_unchanged_chunks > 0:
            _append_log(
                domain, f"Chunks sin cambios detectados: {skipped_unchanged_chunks}"
            )
        ...
        _persist_job_step(
            ...
            "replace_window",
            "running",
            {"months": sorted(target_months, key=_month_serial)},
        )
        incremental_delta_mode = (
            str(mode or "").lower() == "incremental"
            and wm_filter_updated_at is not None
        )
        _set_state(
            domain,
            {
                ...
                "progress_pct": 55,
                "status_message": "Omitiendo replace window (incremental por watermark)"
                if incremental_delta_mode
                else "Reemplazando ventana",
            },
        )
        if target_months and not incremental_delta_mode:
            _delete_target_window(db, domain, mode, year_from, target_months)
            _delete_target_window_fact(
                db, domain, mode, year_from, close_month, target_months
            )
        elif target_months and incremental_delta_mode:
            _append_log(
                domain,
                "Incremental por watermark: se omite replace_window y se aplica UPSERT directo.",
            )
        _persist_job_step(
            db,
            job_id,
        ...
        _append_log(
            domain,
            f"Meses detectados (manifest): {', '.join(ordered_target_months) or '-'}",
        )
        ...
        _set_state(
            domain,
            {
                "stage": "upserting",
                "progress_pct": 75,
                "status_message": "Aplicando UPSERT",
            },
        )
        ...
        if temp_rows_path is not None or temp_rows_by_month:
            base_chunk_size = _adaptive_chunk_size(
                domain, int(settings.sync_fetch_batch_size or 5000)
            )
            chunk_size = (
                _low_impact_chunk_size(base_chunk_size)
                if low_impact_mode
                else base_chunk_size
            )
            chunk_pause_seconds = (
                _low_impact_chunk_pause_seconds() if low_impact_mode else 0.0
            )
            processed = 0
            chunk: list[dict] = []
            ...
            def _apply_chunk(chunk_rows: list[dict], chunk_key: str) -> int:
                ...
                if not deduped_rows:
                    ...
                    return 0
                if (
                    getattr(settings, "sync_postgres_prefilter_enabled", True)
                    and deduped_rows
                ):
                    deduped_rows = _filter_rows_changed_vs_postgres(
                        db, domain, deduped_rows
                    )
                if persist_staging_rows:
                    _persist_staging_rows(
                        db, job_id, domain, chunk_key, deduped_rows, commit=False
                    )
                if persist_sync_records:
                    rows_inserted += _upsert_sync_records(
                        db, deduped_rows, commit=False
                    )
                changed, unchanged = _upsert_fact_rows(
                    db, domain, deduped_rows, commit=False
                )
                db.commit()
            ...
            if domain == "cartera" and temp_rows_by_month:
                ordered_months = sorted(
                    [
                        ...
                        if (not target_months or m in target_months)
                    ],
                    key=_month_serial,
                )
                    ...
                    with open(month_path, "r", encoding="utf-8") as f:
                        for line in f:
                            ...
                            if len(chunk) >= chunk_size:
                                ...
                                processed_by_month[month_key] = (
                                    processed_by_month.get(month_key, 0) + applied
                                )
                                chunk = []
                                pct = 75 + int(
                                    (processed / max(1, normalized_count)) * 20
                                )
                                _set_state(
                                    domain,
                                    {
            ...
            if target_months:
                rows_for_upsert = [
                    ...
                    if str(r.get("gestion_month") or "") in target_months
                ]
            else:
                rows_for_upsert = []
            ...
            if (
                ...
                rows_for_upsert = _filter_rows_changed_vs_postgres(
                    db, domain, rows_for_upsert
                )
            if persist_sync_records:
                rows_inserted = _upsert_sync_records(db, rows_for_upsert, commit=False)
            rows_upserted, rows_unchanged = _upsert_fact_rows(
                db, domain, rows_for_upsert, commit=False
            )
            if rows_upserted > 0:
                applied_months.update(
                    {
            ...
            if persist_staging_rows:
                _persist_staging_rows(
                    ...
                    job_id,
                    domain,
                    _derive_chunk_key(rows_for_upsert),
                    rows_for_upsert,
                    commit=False,
                )
            db.commit()
        ...
        if domain == "analytics":
            ...
            _set_state(
                domain,
                {
                    ...
                    "progress_pct": 88,
                    "status_message": "Actualizando snapshot analytics",
                },
            )
            _refresh_analytics_snapshot(
                db, mode, year_from, refresh_target_months, normalized_rows
            )
        ...
        if domain in {"cartera", "cobranzas", "analytics"}:
            ...
            if domain in {"cartera", "cobranzas"}:
                ...
                deleted_agg, agg_rows_written = _refresh_cartera_corte_agg(
                    db, refresh_target_months
                )
                corte_duration_sec = round(
                    (datetime.now(timezone.utc) - corte_started_at).total_seconds(), 2
                )
                _append_log(
                    domain,
                    (
                ...
                deleted_cohorte, cohorte_rows_written = _refresh_cobranzas_cohorte_agg(
                    db, refresh_target_months
                )
                cohorte_duration_sec = round(
                    (datetime.now(timezone.utc) - cohorte_started_at).total_seconds(), 2
                )
            ...
            for idx, months_batch in enumerate(semantic_batches, start=1):
                ...
                batch_label = (
                    months_batch[0]
                    if len(months_batch) == 1
                    else f"{months_batch[0]}..{months_batch[-1]}"
                )
                _set_state(
                    domain,
                    {
                ...
                _append_log(
                    domain,
                    f"Capa analytics lote {idx}/{total_batches}: meses={batch_label}",
                )
                b_deleted_dim, b_dim_rows = _refresh_dim_negocio_contrato(db, batch_set)
                b_deleted_rend, b_rend_rows = _refresh_analytics_rendimiento_agg(
                    db, batch_set
                )
                b_deleted_dim_contract, b_dim_contract_rows = (
                    _refresh_dim_contract_month_and_catalogs(db, batch_set)
                )
                ...
                options_rows["rendimiento"] += int(
                    (b_options_rows or {}).get("rendimiento", 0)
                )
                options_rows["anuales"] += int((b_options_rows or {}).get("anuales", 0))

            latest_month_set = (
                {ordered_refresh_target_months[-1]}
                if ordered_refresh_target_months
                else set()
            )
            deleted_anuales, anuales_rows_written = _refresh_analytics_anuales_agg(
                db, latest_month_set
            )
            options_consistency = _mv_options_consistency_report(db)
            options_rebuilt_rows: dict[str, int] = {
                "cartera": 0,
                "cohorte": 0,
                "rendimiento": 0,
                "anuales": 0,
            ...
            agg_duration_sec = round(
                (datetime.now(timezone.utc) - agg_started_at).total_seconds(), 2
            )
            _set_state(
                domain,
                {
            ...
            if (
                domain in {"cartera", "cobranzas"}
                and rows_upserted > 0
                and int(agg_rows_written) == 0
            ):
                ...
                if fallback_months:
                    fb_deleted_agg, fb_agg_rows = _refresh_cartera_corte_agg(
                        db, fallback_months
                    )
                    fb_deleted_cohorte, fb_cohorte_rows = (
                        _refresh_cobranzas_cohorte_agg(db, fallback_months)
                    )
                    fb_deleted_dim, fb_dim_rows = _refresh_dim_negocio_contrato(
                        db, fallback_months
                    )
                    fb_deleted_rend, fb_rend_rows = _refresh_analytics_rendimiento_agg(
                        db, fallback_months
                    )
                    fb_deleted_anuales, fb_anuales_rows = (
                        _refresh_analytics_anuales_agg(db, fallback_months)
                    )
                    fb_deleted_dim_contract, fb_dim_contract_rows = (
                        _refresh_dim_contract_month_and_catalogs(db, fallback_months)
                    )
                    fb_deleted_dim_time, fb_dim_time_rows = _refresh_dim_time(
                        db, fallback_months
                    )
                    ...
                    ordered_refresh_target_months = sorted(
                        refresh_target_months, key=_month_serial
                    )
                    _set_state(
                        domain,
                        {
            ...
            deleted_eerr_agg, eerr_agg_rows = refresh_eerr_monthly_agg(
                db, set(ordered_refresh_target_months), _month_serial
            )
            ...
            if rows_upserted > 0 and int(agg_rows_written) == 0:
                ...
                if fallback_months:
                    fb_del, fb_rows = refresh_eerr_monthly_agg(
                        db, fallback_months, _month_serial
                    )
                    agg_rows_written = int(fb_rows)
                    ordered_refresh_target_months = sorted(
                        set(fallback_months), key=_month_serial
                    )
                    _set_state(
                        domain,
                        {
        ...
        _set_state(
            domain,
            {
                "stage": "analyzing",
                "progress_pct": 98,
                "status_message": "Actualizando estadisticas (ANALYZE)",
            },
        )
        ...
        final_state = {
            ...
            "rows_unchanged": rows_unchanged,
            "throughput_rows_per_sec": float(
                (_state_by_domain.get(domain) or {}).get("throughput_rows_per_sec")
                or 0.0
            ),
            "eta_seconds": 0,
            "current_query_file": _query_file_for(domain),
            ...
            "chunk_key": ",".join(ordered_refresh_target_months)
            if ordered_refresh_target_months
            else "*",
            "skipped_unchanged_chunks": int(skipped_unchanged_chunks),
            "affected_months": ordered_refresh_target_months,
        ...
        _persist_job_step(db, job_id, domain, "bootstrap", "completed")
        _persist_job_step(
            db, job_id, domain, "finalize", "completed", {"duration_sec": duration_sec}
        )
        partition_key = _watermark_partition_key(
            mode, year_from, close_month, close_month_from, close_month_to
        )
        ...
        if cleaned_stg > 0:
            _append_log(
                domain, f"Limpieza staging: {cleaned_stg} filas antiguas removidas"
            )
        if domain == "cartera":
            ...
            invalidated_rend_v2_options = invalidate_endpoint(
                ...
                lambda filters: _invalidate_when_months_overlap(
                    filters, target_months_set, "gestion_month"
                ),
            )
            ...
            invalidated_cohorte_v2_fp = invalidate_endpoint(
                ...
                lambda filters: _invalidate_when_months_overlap(
                    filters, target_months_set, "cutoff_month"
                ),
            )
        ...
        is_cancelled = isinstance(exc, SyncCancelledError) or str(error).lower() in {
            "cancelled",
            "cancelled_by_user",
        }
        failed_state = {
            ...
            "progress_pct": 100,
            "status_message": "Sincronizacion cancelada"
            if is_cancelled
            else "Sincronizacion con error",
            "rows_skipped": int(state_snapshot.get("rows_skipped") or 0),
            "skipped_unchanged_chunks": int(
                state_snapshot.get("skipped_unchanged_chunks") or 0
            ),
        ...
        failed_step = (
            _job_step_from_stage((_state_by_domain.get(domain) or {}).get("stage"))
            or "bootstrap"
        )
        _persist_job_step(
            db,
        ...
        _persist_sync_run(
            db,
            {
                ...
                "throughput_rows_per_sec": float(
                    state_snapshot.get("throughput_rows_per_sec") or 0.0
                ),
                "eta_seconds": 0,
                "current_query_file": _query_file_for(domain),
...
class SyncService:
    ...
    def start(
        ...
        mode = SyncService.resolve_mode(
            domain, year_from, close_month, close_month_from, close_month_to
        )
        if (
            bool(getattr(settings, "sync_safe_mode", True))
            and mode == "full_all"
            and not _domain_has_effective_limit(domain)
        ):
            raise RuntimeError(
        ...
        if (
            mode == "full_month"
            and close_month_from
            and close_month_to
            and close_month_from == close_month_to
        ):
    ...
    def preview(
        ...
        *,
        ...
        if not settings.sync_preview_enabled:
            raise RuntimeError(
                "Preview de sincronizacion deshabilitado por configuracion."
            )
        mode = SyncService.resolve_mode(
            domain, year_from, close_month, close_month_from, close_month_to
        )
        ...
        effective_sample_rows = int(
            sample_rows or settings.sync_preview_sample_rows or 20000
        )
        effective_sample_rows = max(1000, min(200000, effective_sample_rows))
        effective_timeout_sec = int(
            timeout_seconds or settings.sync_preview_sample_timeout_seconds or 8
        )
            ...
            for row in batch:
                ...
                if sample_cutoff is not None and estimated >= sample_cutoff:
                    ...
                    return {
                        ...
                        "max_rows_allowed": max_rows,
                        "would_exceed_limit": bool(
                            max_rows is not None and estimated > max_rows
                        ),
                        "sampled": True,
                        "scan_mode": "sampled",
        ...
        try:
            ...
            try:
                ...
                if row is not None and (
                    str(row.stage or "").lower() == "failed" or bool(row.error)
                ):
                    _mark_queue_job_done(
                        job_id, "failed", str(row.error or "sync_failed")
                    )
    ...
    def status(domain: str, job_id: str | None = None) -> dict:
            ...
            if row is not None:
                ...
                if chunk_row is not None:
                    ...
                    skipped_unchanged = int(
                        details.get("skipped_unchanged_chunks") or 0
                    )
                ...
                if queue_row is not None:
                    ...
                    if queue_status in {"cancelled", "failed", "completed"}:
                        ...
                        if queue_status == "cancelled":
                            ...
                            error_value = (
                                None
                                if queue_error == "cancelled_by_user"
                                else (queue_error or error_value)
                            )
                    ...
                    running_value and str(stage_value or "").strip().lower() == "queued"
                ):
                ...
                db_result = {
                    ...
                    "throughput_rows_per_sec": float(
                        getattr(row, "throughput_rows_per_sec", 0.0) or 0.0
                    ),
                    "eta_seconds": int(getattr(row, "eta_seconds", 0) or 0),
                    "current_query_file": getattr(row, "current_query_file", None)
                    or _query_file_for(domain),
                    "job_step": getattr(row, "job_step", None)
                    or _job_step_from_stage(stage_value),
                    ...
                    "watermark": (
                        {
                            ...
                            "last_updated_at": wm.last_updated_at.isoformat()
                            if wm.last_updated_at
                            else None,
                    ...
                    "chunk_key": getattr(row, "chunk_key", None)
                    if hasattr(row, "chunk_key")
                    else None,
                    "chunk_status": getattr(row, "chunk_status", None)
                    if hasattr(row, "chunk_status")
                    else None,
                    "skipped_unchanged_chunks": int(
                        getattr(row, "skipped_unchanged_chunks", 0) or skipped_unchanged
                    ),
                    "affected_months": [],
                    ...
                    "finished_at": finished_at_value.isoformat()
        ...
        if state and (job_id is None or state.get("job_id") == job_id):
            ...
            state["log"] = (
                [str(x) for x in raw_log] if isinstance(raw_log, list) else []
            )
    ...
    def rebuild_options(scope: str, months: list[str] | None, actor: str) -> dict:
        ...
        try:
            ...
            invalidated = {
                "portfolio_corte_options": invalidate_endpoint(
                    "portfolio/corte/options", lambda _: True
                ),
                "portfolio_corte_v2_options": invalidate_endpoint(
                    "portfolio-corte-v2/options", lambda _: True
                ),
                "cobranzas_cohorte_options": invalidate_endpoint(
                    "cobranzas-cohorte/options", lambda _: True
                ),
                "cobranzas_cohorte_v2_options": invalidate_endpoint(
                    "cobranzas-cohorte-v2/options", lambda _: True
                ),
                "rendimiento_v2_options": invalidate_endpoint(
                    "rendimiento-v2/options", lambda _: True
                ),
                "anuales_v2_options": invalidate_endpoint(
                    "anuales-v2/options", lambda _: True
                ),
                "portfolio_corte_summary": invalidate_endpoint(
                    "portfolio/corte/summary", lambda _: True
                ),
                "portfolio_corte_v2_summary": invalidate_endpoint(
                    "portfolio-corte-v2/summary", lambda _: True
                ),
                "cobranzas_cohorte_summary": invalidate_endpoint(
                    "cobranzas-cohorte/summary", lambda _: True
                ),
                "rendimiento_v2_summary": invalidate_endpoint(
                    RENDIMIENTO_V2_SUMMARY_CACHE_SCOPE, lambda _: True
                ),
                "anuales_v2_summary": invalidate_endpoint(
                    "anuales-v2/summary", lambda _: True
                ),
                "portfolio_corte_first_paint": invalidate_endpoint(
                    "portfolio-corte-v2/first-paint", lambda _: True
                ),
                "cobranzas_cohorte_first_paint": invalidate_endpoint(
                    "cobranzas-cohorte-v2/first-paint", lambda _: True
                ),
                "cobranzas_cohorte_detail": invalidate_endpoint(
                    "cobranzas-cohorte-v2/detail", lambda _: True
                ),
                "rendimiento_v2_first_paint": invalidate_endpoint(
                    "rendimiento-v2/first-paint", lambda _: True
                ),
                "anuales_v2_first_paint": invalidate_endpoint(
                    "anuales-v2/first-paint", lambda _: True
                ),
            }
            return {
    ...
    def reset_watermarks(
        domain: str, query_file: str | None = None, partition_key: str | None = None
    ) -> dict:
        db = SessionLocal()
        try:
                ...
                chunks.append(
                    {
                        ...
                        "throughput_rows_per_sec": float(
                            row.throughput_rows_per_sec or 0.0
                        ),
                        "details": details,
        ...
        try:
            ...
            for row in rows:
                ...
                if str(row.stage or "").lower() == "completed":
                    ...
                    current_finished = (
                        str(current.get("finished_at") or "") if current else ""
                    )
            ...
            if totals["jobs"] > 0:
                ...
                totals["p95_duration_sec"] = (
                    round(_percentile(durations, 0.95), 2) if durations else 0.0
                )
            ...
            for (_, step_name), row in latest_step_rows.items():
                ...
                step_rows_value = details.get(
                    "rows_upserted", details.get("rows_read", details.get("normalized"))
                )
                try:
            ...
            for domain, bucket in by_domain.items():
                ...
                bucket["avg_duration_sec"] = round(
                    float(bucket["_duration_sum"]) / max(1, jobs), 2
                )
                tcount = int(bucket["_throughput_count"] or 0)
                bucket["avg_throughput_rows_per_sec"] = round(
                    float(bucket["_throughput_sum"]) / max(1, tcount), 2
                )
            ...
            for step_name, bucket in by_step.items():
                ...
                bucket["avg_duration_sec"] = (
                    round(sum(durations_for_step) / max(1, len(durations_for_step)), 2)
                    if durations_for_step
                    else 0.0
                )
                bucket["p95_duration_sec"] = (
                    round(_percentile(durations_for_step, 0.95), 2)
                    if durations_for_step
                    else 0.0
                )
                ...
                bucket["avg_rows"] = (
                    round(rows_sum / max(1, rows_count), 2) if rows_count else 0.0
                )
                throughput_count_for_step = int(bucket.pop("_throughput_count", 0) or 0)
                throughput_sum_for_step = float(
                    bucket.pop("_throughput_sum", 0.0) or 0.0
                )
                bucket["avg_throughput_rows_per_sec"] = (
                    round(
                        throughput_sum_for_step / max(1, throughput_count_for_step), 2
                    )
                    if throughput_count_for_step
                    else 0.0
                )
                by_step[step_name] = bucket
            top_slowest_jobs = sorted(
                slowest,
            ...
            return {
                ...
                "last_completed_by_domain": dict(
                    sorted(last_completed_by_domain.items())
                ),
        ...
        try:
            ...
            r = SyncSchedule(
                ...
                next_run_at=_compute_next_run_at(
                    interval_value, interval_unit, from_dt=now
                ),
            )
            db.add(r)
            db.commit()
        ...
        try:
            ...
            if interval_unit is not None:
                if (
                    ...
                    and (r.interval_value if interval_value is None else interval_value)
                    < 10
                ):
                    raise ValueError("El intervalo mínimo es 10 minutos")
            ...
            if (
                ...
                and r.interval_unit == "minute"
                and interval_value < 10
            ):
                raise ValueError("El intervalo mínimo es 10 minutos")
    ...
    def delete_schedule(schedule_id: int) -> bool:
        ...
        try:
            ...
            deleted = (
                db.query(SyncSchedule)
                .filter(SyncSchedule.id == schedule_id)
                .delete(synchronize_session=False)
            )
            db.commit()
            return deleted > 0
    ...
    def run_schedule_now(schedule_id: int) -> dict | None:
        ...
        try:
            ...
            if not s:
                return None
            domains = (
                json.loads(s.domains or "[]")
                if isinstance(s.domains, str)
                else (s.domains or [])
            )
            if not domains:
                return {

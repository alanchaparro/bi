ï»¿// Dashboard Logic with IndexedDB Persistence
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        if (cData || cobData || gData || ctData) {
            ...
            progressText.textContent = 'Restaurando sesiÃ³n...';
    ...
    function updateFileStatus(type, name) {
        ...
        el.querySelector('.status-icon').textContent = 'â';
    ...
    async function processCobranzas(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const rawA = r['AÃ±o'] || r['Ano'] || r['Ao'] || r.year || 2026;
    ...
    function updateLtvSelectionSummary(selUn, selAnio, selFecha, selGestion, selViaCobro) {
        ...
        el.innerHTML = `<strong>SelecciÃ³n actual:</strong> UN: ${labels.un} | AÃ±o: ${labels.anio} | Mes/AÃ±o: ${labels.fecha} | GestiÃ³n: ${labels.gestion} | VÃ­a Cobro: ${labels.viaCobro}`;
    }
    ...
    function applyCobranzasFilters() {
        ...
        const filtered = state.cobranzas.data.filter(r => {
            return selVp.has(String(r.VP || '').trim()) &&
                ...
                selAnio.has(String(r['AÃ±o'] || '').trim()) &&
    ...
    function updateCobranzasUI(data) {
        ...
        data.forEach(r => {
            ...
            // Trend (MM/YYYY)
            const f = `${String(r.Mes).padStart(2, '0')}/${r['AÃ±o']}`;
            trend[f] = (trend[f] || 0) + m;
    ...
    async function calculatePerformance() {
        ...
        console.log("Rendimiento: Iniciando cÃ¡lculo robusto optimizado (V7)...");
        ...
        // 1. Aggregate collections track VÃ­a de Pago (Real)
        const cobAggr = {};
        ...
        for (let i = 0; i < pLen; i++) {
            ...
            if (i % 20000 === 0) {
                progressText.textContent = `Generando estadÃ­sticas: ${i.toLocaleString()} / ${pLen.toLocaleString()}...`;
                await new Promise(r => setTimeout(r, 0));
            }
        }
    ...
    async function calculateCosecha() {
        ...
        console.log("Cosecha: Iniciando cÃ¡lculo de cohortes...");
    ...
    async function calculateGestoresDashboard() {
        ...
        console.log("Gestores: Iniciando cÃ¡lculo de productividad...");
        loading.classList.remove('hidden');
    ...
    function updatePerformanceUI(stats) {
        ...
        // NEW: Charts per VÃ­a de Cobro (IntenciÃ³n)
        const vcLabels = Object.keys(viaCStats).sort();
        const vcEff = vcLabels.map(v => (viaCStats[v].d > 0 ? (viaCStats[v].p / viaCStats[v].d) * 100 : 0));
        renderChart('perfViaCobroChart', 'rendimiento', 'bar', vcLabels, vcEff, '% Eficacia (IntenciÃ³n)', '#10b981');
    ...
    function updateCosechaUI(stats) {
        ...
        renderChart('coRecoveryChart', 'cosecha', 'line', sortedMonths, data, '% RecuperaciÃ³n', '#f59e0b');

        const dataCount = sortedMonths.map(m => (trend[m].cBase > 0 ? (trend[m].cPaid / trend[m].cBase) * 100 : 0));
        renderChart('coRecoveryCountChart', 'cosecha', 'line', sortedMonths, dataCount, '% RecuperaciÃ³n (Cantidad)', '#22c55e');
    ...
    function updateLtvUI(stats) {
        ...
        document.getElementById('ltv-financial-detail').textContent =
            `%Pago: ${(stats.ratioPago * 100).toFixed(1)}% | Cobrado: ${formatPYG(stats.sumCobrado)} | DeberÃ­a: ${formatPYG(stats.sumDeberia)} | Meses: ${stats.mesesPonderados.toFixed(2)} | Corte: ${stats.ltvCutoff}`;
    ...
    async function runExport(type) {
        addLog(`Iniciando generaciÃ³n de ${type} desde SQL...`, 'system');
        ...
        try {
            ...
            if (data.success) {
                addLog(`ExportaciÃ³n de ${type} terminada con Ã©xito.`, 'success');
            } else {
                addLog(`Error en exportaciÃ³n: ${data.message}`, 'error');
            }
        } catch (err) {
            addLog(`Error de conexiÃ³n con el servidor local.`, 'error');
        } finally {
            btn.disabled = false;
        }
    }
    ...
    async function syncLocalFiles() {
        ...
        addLog('Iniciando sincronizaciÃ³n de archivos locales...', 'system');
        ...
        try {
            ...
            addLog('SincronizaciÃ³n completada.', 'success');
        } catch (err) {
            ...
            if (files.cartera || files.cobranzas || files.gestores || files.contratos) {
                ...
                hint.innerHTML = `
                    ...
                    <div style="display:flex; gap:1rem; justify-content:center">
                        ...
                        <button onclick="enterConfig()" class="btn-secondary" style="font-size:0.8rem">ConfiguraciÃ³n</button>
                    </div>
                `;
    ...
    window.clearAllData = async () => {
        if (!confirm('Â¿Seguro que deseas borrar toda la memoria del navegador y reiniciar?')) return;
        const db = await openDB();
        const tx = db.transaction(storeName, 'readwrite');

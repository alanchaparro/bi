# Design System: Linear (EPEM Analytics Edition)
...
The color system is almost entirely achromatic — dark backgrounds with white/gray text — punctuated by a single brand accent: **EPEM's signature emerald green** (`#10b981` for backgrounds, `#00c573` for interactive accents). This accent color is used sparingly and intentionally, appearing only on CTAs, active states, and brand elements. The border system uses ultra-thin, semi-transparent white borders (`rgba(255,255,255,0.05)` to `rgba(255,255,255,0.08)`) that create structure without visual noise, like wireframes drawn in moonlight.
...
- Brand emerald-green: `#10b981` (bg) / `#00c573` (accent) / `#00e5a4` (hover) — the only chromatic color in the system
...
### EPEM Specific Notes
- This design system is customized for **EPEM - Cartera de Cobranzas** analytics frontend
- Color de acento: verde emerald canónico alineado con AGENTS.md y docs/spec-canon-patrones-ui-analytics.md
- Compatible con HeroUI y @heroui/react para implementación en Next.js
- No tocar UI legacy (desacople.md) — este sistema es solo para frontend nuevo (v2)
...
### Brand & Accent (EPEM Customized)
- **Brand Emerald** (`#10b981`): Primary brand color — used for CTA button backgrounds, brand marks, and key interactive surfaces. Aligned with canónico EPEM.
- **Accent Green Link** (`#00c573`): Brighter variant for interactive elements — links, active states, selected items.
- **Accent Hover** (`#00e5a4`): Lighter, more saturated variant for hover states on accent elements.
- **Success Green** (`#27a644`): Primary success/active status. Used for "in progress" indicators and KPIs positivos.

### Status Colors
- **Success** (`#27a644`): Primary success/active status. Used for "in progress" indicators.
...
**Primary Brand Button (EPEM Emerald)**
...
- CTA: Brand emerald button or ghost button
...
- Reserve brand emerald (`#10b981` / `#00c573`) for primary CTAs and interactive accents only
...
- Don't apply the brand emerald decoratively — it's reserved for interactive/CTA elements only
...
- Don't introduce warm colors into the UI chrome — the palette is cool gray with blue-violet accent only (emerald accent only for EPEM)
...
- Product screenshots use responsive sizing with consistent border radius
- Dark background ensures screenshots blend naturally at any viewport

## 9. Agent Prompt Guide

### Quick Color Reference
- Primary CTA: Brand Emerald (`#10b981`)
...
- "Create navigation: dark sticky header on `#0f1011`. Inter Variable 13px weight 510 for links, `#d0d6e0` text. Brand emerald CTA `#10b981` right-aligned with 6px radius. Bottom border: `1px solid rgba(255,255,255,0.05)`."
...
5. Brand emerald (`#10b981` / `#00c573`) is the only chromatic color — everything else is grayscale
...
## 10. EPEM-Specific Integration Notes

### 10.1. Alineación con Canónicos
- Este DESIGN.md está alineado con `AGENTS.md` y `docs/spec-canon-patrones-ui-analytics.md`
- Color de acento verde emerald (`#10b981`) respeta canónico EPEM de cobranzas
- No tocar UI legacy — usar solo para frontend nuevo (v2)

### 10.2. Compatibilidad con HeroUI
- Este sistema de diseño es compatible con HeroUI (`@heroui/react`)
- Usar tokens de Tailwind para implementar los colores
- Validar que no haya marcadores de UI legacy en componentes migrados

### 10.3. Validación de Rendimiento
- Monitorear p95 en endpoints de analytics (objetivo: ≤1200ms)
- Validar que cambios visuales no degradan rendimiento
- Revisar `perf_current.json` antes/después de cambios

### 10.4. Reglas de Seguridad
- No commitear `.env` ni secretos
- Escaneo `gitleaks` antes de cada commit
- No hardcodear colores en componentes; usar CSS variables o Tailwind config

### 10.5. Backlogs de Seguimiento
- Hallazgos visuales: `bugs_visual.md`
- Optimización UX: `optimo.md`
- Corridas QA: `qa.md`
- Voz de cliente no técnico: `pendientes.md`

---

*Documento canónico para frontend analytics de EPEM — todo cambio debe validarse contra `AGENTS.md` antes de mergear.*
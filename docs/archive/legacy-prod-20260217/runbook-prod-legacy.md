...
## Sync analytics desde legacy (MySQL)
Objetivo: mantener produccion alineada con legacy sin recarga full en cada corrida.

Variables:
- `ANALYTICS_SYNC_MODE=incremental` (default recomendado)
- `ANALYTICS_SYNC_WINDOW_MONTHS=3` (reemplaza solo los ultimos 3 meses detectados)

Ejecucion:
1. `python scripts/sync_analytics_to_snapshot.py`
2. Verificar salida: `mode=<...> months=<...>` y cantidad de filas sincronizadas.

Recarga completa (solo mantenimiento/correccion mayor):
- `ANALYTICS_SYNC_MODE=full python scripts/sync_analytics_to_snapshot.py`

Programacion automatica en Windows (cada 15 min):
- Script: `powershell -ExecutionPolicy Bypass -File .\scripts\run_sync_incremental.ps1`
- Crear tarea:
`schtasks /Create /SC MINUTE /MO 15 /TN "CobranzasSyncIncremental" /TR "powershell -NoProfile -ExecutionPolicy Bypass -File C:\desarrollos\cartera_cobranzas_auto\scripts\run_sync_incremental.ps1" /F`
...
2. Ejecutar `scripts/sync_analytics_to_snapshot.py` en modo incremental.
3. Validar `/api/v1/health` y `/api/check-files`.

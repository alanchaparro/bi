...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function calculateAcaAnuales() {
        ...
        for (let i = 0; i < state.cartera.data.length; i++) {
            ...
            if (!byContractMonth[key]) {
                byContractMonth[key] = {
                    ...
                    month: fe,
                    cuotaSum: cuotaNum,
                    cuotaCount: Number.isFinite(cuotaRaw) ? 1 : 0
                };
                continue;
            }

            const curr = byContractMonth[key];
            curr.cuotaSum += cuotaNum;
            if (Number.isFinite(cuotaRaw)) curr.cuotaCount += 1;
        }
        ...
        const findSnapshotAtOrAfter = (cId, month) => {
            ...
            for (let i = 0; i < timeline.length; i++) {
                const curr = timeline[i];
                if (monthToSerial(curr.month) >= target) return curr;
            }
            return null;
        };
        ...
        const getCumPaidUpTo = (cId, cutoffSerial) => {
            ...
            while (lo <= hi) {
                const mid = (lo + hi) >> 1;
                if (entry.serials[mid] <= cutoffSerial) {
                    idx = mid;
                    lo = mid + 1;
                } else {
                    hi = mid - 1;
        ...
        const contractStartSerialById = {};
        for (let i = 0; i < state.contratos.data.length; i++) {
            const c = state.contratos.data[i];
            const cId = String(c._cId || '');
            const m = String(c._contractMonth || '');
            const serial = monthToSerial(m);
            if (!cId || serial <= 0) continue;
            if (!contractStartSerialById[cId] || serial < contractStartSerialById[cId]) {
                contractStartSerialById[cId] = serial;
            }
        }
        for (let i = 0; i < timelineIds.length; i++) {
            const cId = timelineIds[i];
            ...
            const firstSerial = monthToSerial(timeline[0].month);
            if (!contractStartSerialById[cId] || firstSerial < contractStartSerialById[cId]) {
                contractStartSerialById[cId] = firstSerial;
            }
        }

        const monthsActiveBetween = (startSerial, endSerial) => {
            if (!Number.isFinite(startSerial) || !Number.isFinite(endSerial)) return 0;
            if (endSerial < startSerial) return 0;
            const diff = endSerial - startSerial;
        ...
        for (let i = 0; i < years.length; i++) {
            ...
            for (let j = 0; j < yearContracts.length; j++) {
                ...
                if (Number.isFinite(tramoCutoff) && tramoCutoff <= 3) {
                    contractIdsVigentes.add(cId);
                }
            ...
            const contractsVigentes = contractIdsVigentes.size;
            const tkpContrato = contracts > 0 ? (cuotaTotal / contracts) : 0;
            let paidToCutoffTotal = 0;
            let txToCutoffTotal = 0;
            let activeMonthsTotal = 0;
            ...
            for (let j = 0; j < contractIdList.length; j++) {
                ...
                const startSerial = contractStartSerialById[cId] || cutoffSerial;
                activeMonthsTotal += monthsActiveBetween(startSerial, cutoffSerial);
                ...
                if (byMonth) {
                    for (const key in byMonth) {
                        ...
                        const amount = bucket && Number.isFinite(bucket.amount) ? bucket.amount : 0;
                        paidByContractMonthTotal += amount;
                        paidByContractMonthCount += 1;
                    }
                }
            }

            const tkpTransaccional = txToCutoffTotal > 0 ? (paidToCutoffTotal / txToCutoffTotal) : 0;
            const tkpPago = paidByContractMonthCount > 0 ? (paidByContractMonthTotal / paidByContractMonthCount) : 0;

            const culminadosContracts = yearContracts;
            let culminados = 0;
            ...
            for (let j = 0; j < culminadosContracts.length; j++) {
                const c = culminadosContracts[j];
                const cId = String(c._cId || '');
                const culmMonth = String(c._culminacionMonth || '');
                const culmSerial = monthToSerial(culmMonth);
                ...
                const cuotaCul = snap
                    ? Number(snap.cuotaAvg || 0)
                    : (parseFloat(c._montoCuota || c.monto_cuota || c.amount || 0) || 0);
            ...
            const tkpContratoCulminado = culminados > 0 ? (cuotaCulTotal / culminados) : 0;
            const tkpPagoCulminado = paidByContractMonthCulCount > 0 ? (paidByContractMonthCulTotal / paidByContractMonthCulCount) : 0;
            const tkpContratoCulminadoVigente = culminadosVigentes > 0 ? (cuotaCulTotalVigente / culminadosVigentes) : 0;
            const tkpPagoCulminadoVigente = paidByContractMonthCulCountVigente > 0
                ? (paidByContractMonthCulTotalVigente / paidByContractMonthCulCountVigente)
                : 0;
            const ltvCulminadoVigente = totalDeberiaCulVigente > 0
                ...
                const movementLineLabelsPlugin = {
                    ...
                    afterDatasetsDraw(chart) {
                        ...
                        for (let i = 0; i < points.length; i++) {
                            ...
                            const labelY = Math.max(chartArea.top + box.h, p.y - 8);
                            drawBubble(text, p.x, labelY);
                        }
                        ctx.restore();
                    }
                };
